...
public class Complex extends Object {
    ...
    public static final Complex ZERO = new Complex(0, 0),
            ...
            TWO = new Complex(2, 0),
            THREE = new Complex(3, 0),
            FOUR = new Complex(4, 0),
            FIVE = new Complex(5, 0);
    ...
    public static Complex fromPolar(double rad, double ang) {
    ...
    /**
     * Raising a base r to this number.
     * @param r is the base
     * @return new Complex number r^z where z is this complex number.
     */
    public Complex expBase(double r) {
        throw new UnsupportedOperationException("Not sure if pow(double) [Complex] works!");
//        return mult(new Complex(Math.log(r), 0)).exp();
    }

    /**
     * Exponentiation where the power is an integer.
     * @param w integer
     * @return z^w where z is this complex and w is the parameter
     */
    public Complex pow(int w) {
        if (w == 0)
            return ONE;
        else if (w == 1)
            return this;
        else if (w > 1) {
            Complex c = this.mult(this);
            for (int i = 2; i < w; i++) {
                c = c.mult(this);
            }
            return c;
        } else { //num < 0
            return ONE.div(pow(-w));
//            Complex c = ONE.div(this);
//            for(int i = -2; i > num; i--) {
//                c = c.div(this);
//            }
//            return c;
