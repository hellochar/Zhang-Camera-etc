...
/**
 * This is meant to override the default RMISocketFactory.  The default factory provides sockets
 * with very long timeouts, resulting in approx. 5-minute hang delays when connections are attempted
 * to IP addresses that are currently inaccessible (offline or non-existent on subnet).  This new
 * implementation provides shorter timeouts, approx 30-seconds.
 * <p>
 * Not 100% sure of resulting delay... it may be multiplied by 3 in the end, since RMI may try three
 * ways: 1) socket  2) html  3) cgi  (per Sun javadoc)
 * <p>
 * To use, just instantiate once within your JVM, prerably before any RMI code.  (Additional attempts will throw exceptions.)
 * <p>
 * A map of proxy redirects, and/or a single default proxy is supported.
 * See setProxies() and setProxy() for more info.
 *
 * @author  Chris.Janicki@AugurSystems.com
 */
public class ASIRmiSocketFactory extends RMISocketFactory
{
...
/**
 * Creates a new instance of ASIRmiSocketFactory with a default 10-sec timeout,
 * and installs itself as the new default; can only happen once, else an
 * exception is thrown.
 */
	 ...
	 *
	 * @param server  The InetAddress of the network interface on which to bind server
	 * sockets; if null, will bind to all interfaces.
	 */
	public ASIRmiSocketFactory(InetAddress server, int timeoutMillis) throws IOException
	{
	 ...
	 * server's RMI Registry JVM, but if that same Registry is used
	 * by other servers on the same protected network, then their stub connections
	 * would be routed through the external NAT address, which is not efficient
	 * or secure.  Note that this is only an issue in a client-server system
	 * where there are multiple networked servers.  A stand-alone system is fine just
	 ...
	 *
	 * @param proxies  A Map, possibly null, whose keys are the String IP addresses
	 * of the protected servers; the values are the associated public NAT addresses.
	 * The proxy map will be used whenever the createSocket() method is called.
	 * The proxies do not affect the createServerSocket() method.
	 */
